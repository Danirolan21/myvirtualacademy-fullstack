-- =============================================================================
-- 005_identity_migration.sql
-- Convierte 8 tablas a IDENTITY mediante table-swap preservando IDs existentes.
--
-- Tablas afectadas:
--   Usuarios, Cursos, Asignaturas, Temas, Contenidos,
--   Comentarios_Calificaciones, Entregas_Tareas, Historial_Calificaciones
--
-- Vistas refrescadas con sp_refreshview tras el cambio:
--   Vista_Usuarios_Con_Roles, Vista_Inscripciones, Vista_Asignaturas_Usuario,
--   Vista_Cursos_Detalles, Vista_Detalles_Asignatura
--
-- Estrategia: drop dinámico de todas las FKs entrantes, sp_rename → CREATE TABLE
-- con IDENTITY → IDENTITY_INSERT ON/INSERT/OFF → DBCC CHECKIDENT RESEED.
-- Todo en una sola transacción con SET XACT_ABORT ON y BEGIN TRY/CATCH.
-- =============================================================================
...
BEGIN TRY
    ...
    PRINT 'FKs entrantes detectadas para drop: ' + CAST(@fk_count AS NVARCHAR(10));
    ...
    PRINT 'FKs dropeadas correctamente: ' + CAST(@fk_count AS NVARCHAR(10));

    -- Índices únicos de Usuarios
    IF EXISTS (
        SELECT 1 FROM sys.indexes
        WHERE name = 'IX_Usuarios_Telefono' AND object_id = OBJECT_ID('dbo.Usuarios')
    )
        DROP INDEX [IX_Usuarios_Telefono] ON [dbo].[Usuarios];

    DECLARE @uq_email SYSNAME;
    SELECT @uq_email = kc.name
    FROM sys.key_constraints kc
    WHERE kc.parent_object_id = OBJECT_ID('dbo.Usuarios')
      AND kc.type = 'UQ';
    IF @uq_email IS NOT NULL
        EXEC('ALTER TABLE [dbo].[Usuarios] DROP CONSTRAINT ' + QUOTENAME(@uq_email));
